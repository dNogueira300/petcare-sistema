...
export function BookingModal({ open, onClose, mascotas, onBooked, prefillMotivo, prefillMascotaId }: Props) {
  ...
  const btnDisabled = submitting || !hora || !idMascota || !motivo;
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:"16px" }}>
      ...
      <div style={{ position:"relative", zIndex:1, width:"100%", maxWidth:"520px", background:"#fff", borderRadius:"24px", overflow:"hidden", boxShadow:"0 32px 80px rgba(10,26,17,0.4)", maxHeight:"90vh", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ flex:1, minHeight:0, overflowY:"auto", padding:"24px" }}>
          {step === "success" ? (
            <div style={{ textAlign:"center", padding:"16px 0" }}>
              ...
              <p style={{ fontFamily:"var(--font-dm-sans)", fontSize:"0.88rem", color:"#8a7a60", lineHeight:1.6, margin:0 }}>Tu cita fue agendada exitosamente. Recuerda llegar 10 minutos antes de tu hora.</p>
            </div>
          ) : (
            <form id="booking-form" onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:"18px" }}>
              {/* Mascota */}
        ...
        <div style={{ flexShrink:0, borderTop:"1px solid #f0ead8", padding:"14px 24px 18px", background:"#fff" }}>
          {step === "success" ? (
            <button onClick={onClose}
              style={{ width:"100%", background:"#0a1a11", color:"#fff", border:"none", cursor:"pointer", padding:"14px", borderRadius:"12px", fontSize:"0.9rem", fontWeight:700, fontFamily:"var(--font-dm-sans)" }}>
              Cerrar
            </button>
          ) : (
            <button type="submit" form="booking-form" disabled={btnDisabled}
              style={{ width:"100%", background:"linear-gradient(135deg,#c48c34,#a07028)", color:"#fff", border:"none", cursor:btnDisabled?"not-allowed":"pointer", padding:"14px", borderRadius:"12px", fontSize:"0.9rem", fontWeight:700, fontFamily:"var(--font-dm-sans)", opacity:btnDisabled?0.6:1 }}>

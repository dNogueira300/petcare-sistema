...
export default function PortalHistoriaPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const hRes = await fetch(`/api/historia-clinica?id_mascota=${id}`).then((r) => r.json()).catch(() => ({}));
      if (hRes?.error) { setDenegado(true); setLoading(false); return; }
      ...
      <main style={{ maxWidth: "880px", margin: "0 auto", padding: "24px clamp(12px,3vw,32px)" }}>
        <div style={{ background: "#faf8f3", border: "1px solid #e0d8ca", borderRadius: "20px", padding: "24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "14px", marginBottom: "20px", flexWrap: "wrap" }}>
            <div style={{ width: "52px", height: "52px", borderRadius: "14px", background: "linear-gradient(135deg,#dbeafe,#bfdbfe)",
          ...
          {loading ? (
            ...
            <p style={{ fontFamily: "var(--font-dm-sans)", color: "#991b1b" }}>No tienes acceso a esta historia.</p>
          ) : historias.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 20px", color: "#a89a80",
              ...
              {historias.map((h) => (
                <div key={h.id_historia} style={{ background: "#fff", border: "1px solid #e8e0d0",
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start",
                    ...
                    <div>
                      <p style={{ fontFamily: "var(--font-dm-sans)", fontWeight: 700, color: "#1a1208", margin: 0, fontSize: "0.95rem" }}>
                    ...
                    {h.peso_consulta && (
                      <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: "0.78rem", fontWeight: 600,
                        color: "#3d845b", background: "#f0fdf4", padding: "4px 10px", borderRadius: "99px", whiteSpace: "nowrap" }}>
                        {h.peso_consulta} kg
                      </span>
                    )}
                  </div>
